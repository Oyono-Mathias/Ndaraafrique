...
export async function createQuiz({ courseId, sectionId, formData }: { courseId: string; sectionId: string; formData: unknown }) {
  ...
  try {
    const db = getAdminDb();
    const newQuizRef = db.collection('courses').doc(courseId).collection('sections').doc(sectionId).collection('quizzes').doc();
...
export async function updateQuiz({ courseId, sectionId, quizId, formData }: { courseId: string; sectionId: string; quizId: string; formData: unknown }) {
  ...
  try {
    const db = getAdminDb();
    const quizRef = db.collection('courses').doc(courseId).collection('sections').doc(sectionId).collection('quizzes').doc(quizId);
