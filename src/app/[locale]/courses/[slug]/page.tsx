...
function CoursePlayerPageContent() {
  ...
  // Real-time Discussion Listener
  useEffect(() => {
      if (!activeLecture?.id) return;
      
      const q = query(
          collection(db, 'lesson_questions'),
          where('lessonId', '==', activeLecture.id),
  ...
  const handleReply = async (questionId: string, studentId: string) => {
      ...
      const result = await replyToLessonQuestion({
          questionId,
              ...
              <div className="w-full max-w-6xl mx-auto">
                {activeLecture.type === 'video' ? (
                  activeLecture.contentUrl?.includes('youtube') || activeLecture.contentUrl?.includes('youtu.be') ? (
                    ...
                    <div className="w-full aspect-video bg-slate-900 overflow-hidden shadow-2xl relative">
                        <video 
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              {activeLecture && (
                <Button 
                  ...
                  className={cn(
                    "h-11 px-8 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all shadow-xl shadow-primary/20",
                    completedLessons.includes(activeLecture.id) ? "bg-green-500/10 text-green-500 border-green-500/20" : "bg-primary text-white"
                  )}
                >
                          ...
                          <div key={q.id} className="bg-slate-900/50 border border-slate-800 rounded-[2rem] p-6 space-y-6 shadow-xl animate-in fade-in duration-500">
                              ...
                              <div className="flex items-start gap-4">
                                  <Avatar className="h-10 w-10 border border-white/10">
                                      ...
                                      <AvatarFallback className="bg-slate-800 text-slate-500 font-bold">{q.studentName?.charAt(0)}</AvatarFallback>
                                  </Avatar>
                                  <div className="flex-1">
                                      <div className="flex justify-between items-baseline mb-1">
                                          ...
                                          <span className="text-[10px] font-black uppercase text-slate-600 tracking-widest">
                                              {q.createdAt && (q.createdAt as any).toDate ? format((q.createdAt as any).toDate(), 'dd MMM HH:mm', { locale: fr }) : '...'}
                                          </span>
                                      </div>
                              ...
                              {q.replies && q.replies.length > 0 && (
                                  <div className="space-y-4 pl-8 border-l border-primary/20 mt-4">
                                      {q.replies.map((reply: any, idx: number) => (
                                          <div key={idx} className="flex items-start gap-3 bg-primary/5 p-4 rounded-2xl border border-primary/10">
                                              <div className="p-2 bg-primary/20 rounded-xl">
                                                  <Bot className="h-4 w-4 text-primary" />
                                              </div>
                                              <div>
                                                  <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-1">{reply.instructorName}</p>
                              ...
                              {isInstructor && (
                                  <div className="pt-4 border-t border-white/5">
                                      <div className="flex gap-2">
                                          <Input 
                                              placeholder="Votre réponse d'expert..." 
                                              className="h-11 bg-slate-950 border-slate-800 rounded-xl text-xs"
                                              value={replyText[q.id] || ''}
                                              onChange={(e) => setReplyText(prev => ({ ...prev, [q.id]: e.target.value }))}
                                          />
                                          <Button 
                                              size="sm" 
                                              className="h-11 rounded-xl bg-primary text-white font-black px-4"
                                              disabled={isReplying === q.id || !replyText[q.id]?.trim()}
                                              onClick={() => handleReply(q.id, q.studentId)}
                                          >
                                              {isReplying === q.id ? <Loader2 className="h-4 w-4 animate-spin"/> : <Reply className="h-4 w-4" />}
                                          </Button>
                                      </div>
                                  </div>
                              )}
