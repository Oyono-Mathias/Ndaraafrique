...
/**
 * @fileOverview Lecteur de cours pour les étudiants de Ndara Afrique.
 * Gère l'affichage multimédia, le tracking de progression et l'IA Mathias.
 */
...
import { Loader2, CheckCircle, Bot, Play, Info, ArrowLeft } from 'lucide-react';
...
// Chargement dynamique du player pour éviter les erreurs SSR
...
function CoursePlayerPageContent() {
  ...
  // 2. Chargement structuré du contenu du cours
  useEffect(() => {
    if (!courseId) return;
    const fetchCourseContent = async () => {
      ...
      try {
        ...
        // Sélection automatique de la première leçon si aucune n'est active
        if (!activeLecture && fetchedSections.length > 0) {
          const firstSectionLectures = lecturesData.get(fetchedSections[0].id);
  ...
  const handleLessonComplete = useCallback(async () => {
    ...
    if (!completedLessons.includes(activeLecture.id)) {
      ...
      toast({ title: "Leçon validée !", description: "Votre progression avance." });
  ...
  // 4. Redirection vers Mathias avec contexte
                ...
                {/* --- BARRE DE CONTRÔLES --- */}
                 <div className="p-6 bg-[#121212] border-t border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
                    <div className="flex-1 overflow-hidden">
                        <h1 className="font-bold text-xl text-white truncate">{activeLecture?.title || 'Choisissez une leçon'}</h1>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        {activeLecture && (
                           ...
                           >
                               ...
                               {completedLessons.includes(activeLecture.id) ? "Validée" : "Marquer comme terminée"}
                           </Button>
                        )}
                    </div>
                 </div>
