 ...
 * Permet aux étudiants de trouver leurs collègues, de filtrer par domaine et de chatter.
 */
...
export default function AnnuairePage() {
    ...
    return (
        <div className="flex flex-col gap-0 pb-24 bg-ndara-bg min-h-screen relative">
            ...
            <header className="sticky top-0 z-40 bg-ndara-bg/95 backdrop-blur-md border-b border-white/5 safe-area-pt">
                <div className="px-6 py-6 flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="icon" className="rounded-full bg-ndara-surface text-slate-400">
                        <Filter className="h-5 w-5" />
                    </Button>
                </div>

                {/* Barre de Recherche Style Qwen */}
                <div className="px-6 pb-4">
                    <div className="relative group">
                        ...
                        <Input 
                            ...
                            className="h-14 pl-14 pr-12 bg-ndara-surface border-white/5 rounded-[2rem] text-white placeholder:text-slate-600 focus-visible:ring-primary/30 shadow-xl"
                            value={searchTerm}
                            ...
                            <button 
                                ...
                                className={cn(
                                    ...
                                    selectedDomain === cat.id 
                                        ...
                                        : "bg-ndara-surface border-white/5 text-slate-500"
                                )}
                            >
            ...
            <main className="px-6 pt-6 space-y-8">
                ...
                <section className="grid grid-cols-3 gap-3">
                    <div className="bg-ndara-surface rounded-3xl p-4 border border-white/5 text-center shadow-xl">
                ...
                <div className="space-y-4">
                    ...
                    {isLoading ? (
                        <div className="space-y-4">
                            {[...Array(4)].map((_, i) => (
                                <div key={i} className="p-5 flex items-center gap-4 bg-ndara-surface/50 rounded-[2.5rem] border border-white/5">
                        ...
                        <div className="grid gap-4 animate-in fade-in duration-700">
                            {filteredClassmates.map(member => {
                                ...
                                return (
                                    <div key={member.uid} className="member-card bg-ndara-surface rounded-[2.5rem] p-4 border border-white/5 flex items-center gap-4 shadow-xl active:scale-[0.97] transition-all group">
                                        <div className="relative flex-shrink-0">
                                            ...
                                            {member.isOnline && (
                                                <div className="absolute bottom-0 right-0 w-4 h-4 bg-primary rounded-full border-2 border-ndara-bg shadow-[0_0_10px_rgba(16,185,129,0.5)] animate-pulse" />
                                            )}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                ...
                                                <span className="text-[9px] font-bold text-slate-600 uppercase tracking-tighter truncate max-w-[80px]">
                                                    {member.countryName || 'Afrique'}
                                                </span>
                                            </div>
                                        </div>
                                        <Button 
                                            size="icon" 
                                            className="h-12 w-12 rounded-full bg-primary/10 hover:bg-primary text-primary hover:text-ndara-bg transition-all shadow-lg border border-primary/20 shrink-0 action-btn"
