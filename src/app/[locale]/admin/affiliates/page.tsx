 ...
 * Gestion avancée du réseau d'affiliation et des commissions.
 */
...
const mockAffiliates = [
    { id: '1', name: 'Mathias Oyono', email: 'mathias@ndara.com', clicks: 1250, sales: 42, revenue: 850000, earnings: 85000 },
    { id: '2', name: 'Marie Dupont', email: 'marie@africa.net', clicks: 840, sales: 15, revenue: 320000, earnings: 32000 },
    { id: '3', name: 'Jean Koulé', email: 'jean.k@gmail.com', clicks: 210, sales: 4, revenue: 80000, earnings: 8000 },
];
    ...
    return (
        <div className="space-y-8 pb-20 animate-in fade-in duration-700">
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard title="Total Ambassadeurs" value="124" icon={Users2} isLoading={false} />
                <StatCard title="Ventes Réseau" value="1.2M XOF" icon={ShoppingCart} isLoading={false} />
                <StatCard title="Commissions Payées" value="150k XOF" icon={Landmark} isLoading={false} />
                <StatCard title="Portée (Clics)" value="45.2k" icon={MousePointer2} isLoading={false} />
            </section>
            ...
            <Tabs defaultValue="list" className="w-full">
                ...
                <TabsContent value="list" className="mt-8 space-y-6">
                    <div className="relative max-w-sm">
                        ...
                        <Input placeholder="Rechercher un ambassadeur..." className="h-12 pl-12 bg-slate-900 border-slate-800 rounded-xl" />
                    </div>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {mockAffiliates.map(aff => (
                                    <TableRow key={aff.id} className="group border-slate-800 hover:bg-slate-800/20">
                                        <TableCell>
                                            <div className="flex items-center gap-3">
                                                <Avatar className="h-10 w-10 border border-slate-700 shadow-lg">
                                                    <AvatarFallback className="bg-slate-800 text-[10px] font-black">{aff.name.charAt(0)}</AvatarFallback>
                                                </Avatar>
                                                <div className="flex flex-col">
                                                    <span className="font-bold text-sm text-white uppercase">{aff.name}</span>
                                        ...
                                        <TableCell>
                                            <div className="flex gap-4">
                                                <div>
                                                    ...
                                                    <p className="text-xs font-black text-white">{aff.clicks}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] text-slate-500 font-bold uppercase">Ventes</p>
                                                    <p className="text-xs font-black text-primary">{aff.sales}</p>
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-right">
                                            <div className="flex flex-col">
                                                <span className="text-sm font-black text-white">{aff.revenue.toLocaleString('fr-FR')} XOF</span>
                                                <span className="text-[9px] font-bold text-emerald-500 uppercase">Comm: {aff.earnings.toLocaleString('fr-FR')} XOF</span>
                                            </div>
                                        </TableCell>
