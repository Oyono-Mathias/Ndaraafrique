    ...
    return (
        <>
            <div className="flex flex-col h-screen bg-gray-50 -m-6">
                <CardHeader className="flex flex-row items-center gap-4 border-b bg-white">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500">
                            Par {ticketUser?.fullName || '...'} - Dernière activité il y a {ticket?.updatedAt ? formatDistanceToNow(ticket.updatedAt.toDate(), { locale: fr }) : ''}
                        </p>
                    </div>
                </CardHeader>
                        ...
                        {messages?.map(message => {
                            ...
                            return (
                                <div key={message.id} className={cn("flex items-start gap-3", isFromAdmin && "justify-end")}>
                                    ...
                                    <div className={cn("rounded-2xl p-3 max-w-[75%] shadow-sm", isFromAdmin ? "bg-primary text-primary-foreground rounded-br-none" : "bg-white border rounded-bl-none")}>
                ...
                <CardFooter className="bg-white border-t p-4 space-y-4 flex-col items-stretch">
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={handleCloseTicket}><XCircle className="mr-2 h-4 w-4" /> Fermer le ticket</Button>
                            <Button variant="outline" size="sm" onClick={() => router.push(`/admin/users/${ticket?.userId}`)}><User className="mr-2 h-4 w-4" /> Voir le profil</Button>
                        </div>
