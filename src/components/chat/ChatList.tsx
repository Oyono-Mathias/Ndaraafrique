...
import { MessageSquare, Search, UserPlus, MoreVertical, ShieldAlert } from 'lucide-react';
...
export function ChatList({ selectedChatId }: { selectedChatId: string | null }) {
    ...
    useEffect(() => {
        ...
        // Requête temps réel : écouter les conversations où l'utilisateur participe
        ...
        const unsubscribe = onSnapshot(chatsQuery, async (snap) => {
            ...
            // Batch fetch des profils des interlocuteurs
        ...
        <div className="flex flex-col h-full bg-slate-950">
            ...
            <ScrollArea className="flex-1">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-12 text-center opacity-20 h-full mt-20">
                        ...
                        {!isAdmin && <p className="mt-2 text-sm max-w-[200px]">Démarrez une conversation en consultant l'annuaire.</p>}
                    </div>
                )}
            </ScrollArea>
            ...
            {!isAdmin && (
                <Button 
                    ...
                    className="fixed bottom-6 right-6 h-14 w-14 rounded-full bg-primary hover:bg-primary/90 shadow-2xl shadow-primary/40 z-50 transition-transform active:scale-90 md:hidden border-none"
                >
