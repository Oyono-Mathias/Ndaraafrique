...
import type { Payment, AssignmentSubmission } from '@/lib/types';
...
export function InstructorDashboard() {
    ...
    useEffect(() => {
        ...
        // 1. Revenus totaux (Encaissé via Moneroo)
        const unsubPayments = onSnapshot(
            ...
            (snap) => {
                ...
                // Calcul de la performance par cours
        ...
        // 2. Nombre total d'étudiants (collection users, rôle student)
        ...
        // 3. Devoirs en attente (graded: false / status: submitted)
            ...
            <section className="px-4 space-y-4">
                <div className="flex items-center justify-between px-1">
                    ...
                    {pendingSubmissions.length > 0 && (
                        <Badge className="bg-primary text-white border-none rounded-full px-2">{pendingSubmissions.length}</Badge>
                    )}
                </div>
            ...
            <section className="px-4 grid grid-cols-1 gap-3">
                <Button variant="outline" asChild className="h-16 justify-between bg-slate-900 border-slate-800 rounded-[1.5rem] group active:scale-95 transition-all">
                    <span className="flex items-center gap-4">
                        ...
                        <span className="font-bold text-slate-200">Gérer mes cours</span>
                    </span>
                ...
                <Button variant="outline" asChild className="h-16 justify-between bg-slate-900 border-slate-800 rounded-[1.5rem] group active:scale-95 transition-all">
                    <span className="flex items-center gap-4">
                        ...
                        <span className="font-bold text-slate-200">Liste des étudiants</span>
                    </span>
                    <ChevronRight className="h-5 w-5 text-slate-700" />
                </Button>
            </section>
