...
export function QnaClient() {
  ...
  // 1. Fetch instructor's courses for the filter dropdown
  ...
  // 2. Fetch all questions for the instructor
  ...
  // 3. Filter questions client-side
        ...
        <CardContent className="p-4 space-y-4">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-full sm:w-[180px] h-11 text-base dark:bg-slate-800"><SelectValue placeholder="Filtrer par statut..." /></SelectTrigger>
              <SelectContent>
          ...
          {isLoading ? (
            ...
            <Accordion type="single" collapsible className="w-full space-y-3 pt-4">
                {filteredQuestions.map(q => (
                     <AccordionItem key={q.id} value={q.id} className="border dark:border-slate-700 rounded-lg overflow-hidden">
                        <AccordionTrigger className="p-4 hover:no-underline hover:bg-slate-800/50 [&[data-state=open]&gt;svg.chevron]:text-primary">
                            ...
                            <Badge variant={getStatusVariant(q.status)} className="ml-4 capitalize">{q.status === 'pending' ? 'En attente' : 'Répondu'}</Badge>
                        </AccordionTrigger>
                        <AccordionContent className="p-4 pt-0 bg-slate-800/30">
                            {q.status === 'answered' && q.answerText && (
                                <div className="mb-4 p-3 border-l-4 border-green-500 bg-green-500/10 rounded-r-lg">
                                    <p className="font-semibold text-sm text-green-300">Votre réponse :</p>
                                    <p className="text-sm text-slate-300 whitespace-pre-wrap">{q.answerText}</p>
                                </div>
                            )}
            ...
            <div className="text-center py-20 border-2 border-dashed border-slate-700 rounded-xl mt-4">
                ...
                <h3 className="mt-4 text-lg font-semibold text-slate-300">Aucune question trouvée</h3>
                <p className="mt-1 text-sm text-muted-foreground">Les questions de vos étudiants apparaîtront ici.</p>
            </div>
          )}
        </CardContent>
      </Card>
